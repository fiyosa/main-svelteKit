import { resCatch, resError, resSuccess } from '../../helper/response'
...
export const permissionDestroyRepository = async (event: ApiEvent) => {
  try {
    ...
    const existing = await db.select({ id: permissions.id }).from(permissions).where(eq(permissions.id, id)).limit(1)

    if (existing.length < 1) return resError(t._('not_found', { operator: t._('permission') }), null, 404)

    await db.update(permissions).set({ deleted_at: new Date() }).where(eq(permissions.id, id))
