...
import { permissionResource } from '$core/resources/policy'
...
export const permissionListRepository = async (_event: ApiEvent) => {
  try {
    ...
    return resSuccessData(permissionResource.collection(result), t._('retrieved_successfully', { operator: t._('permission') }))
  } catch (err) {
    return resCatch(err)
  }
}
